import db from '#db';
import { debtBlock } from './economyUtils.js';
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command, text }) => {
    ...
    const debtError = debtBlock(user);
    if (debtError) return msg.reply(debtError);
    ...
    if (cantidad > user.coins) {
      return msg.reply(`ꕥ No tienes suficientes *${monedas}* fuera del banco para apostar, tienes *¥${user.coins.toLocaleString('en-US')} ${monedas}*.`);
    }
    ...
    const mensaje = `「✿」La moneda ha caído en *${capitalize(resultado)}* y has ${acierto ? 'ganado' : 'perdido'} *¥${Math.abs(cambio).toLocaleString('en-US')} ${monedas}*!\n> Tu elección fue *${capitalize(eleccion)}*`;
