...
export default {
  ...
  run: async ({ msg, sock, usedPrefix }) => {
    ...
    const siguiente = Math.min(20000 + currentStreak * 5000, 1015000).toLocaleString('en-US');
    let caption = `> Día *${currentStreak + 1}* » *+¥${siguiente}*`;
    if (lost) caption += `\n> ☆ ¡Has perdido tu racha de días!`;
    await msg.reply(`「✿」Has reclamado tu recompensa diaria de *¥${recompensa.toLocaleString('en-US')} ${monedas}*! (Día *${currentStreak}*)\n${caption}`);
  }
};
