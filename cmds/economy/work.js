...
export default {
  ...
  run: async ({ msg, sock, usedPrefix, command }) => {
    ...
    const reward = Math.floor(Math.random() * (11000 - 7000 + 1)) + 7000;
    let totalReward = reward;
    let bonus = '';
    if (Math.random() < 0.15) {
      const extra = Math.floor(Math.random() * (4500 - 2500 + 1)) + 2500;
      totalReward += extra;
      bonus = `\nꕥ ¡Obtuviste un bono extra de *¥${extra.toLocaleString()} ${monedas}*!`;
    }
    const balanceError = checkBalanceLimit(user, totalReward);
    if (balanceError) {
      return msg.reply(balanceError);
    }
    const debtResult = applyDebtPayment(msg.chat, msg.sender, totalReward, user);
    if (debtResult.error) {
      return msg.reply(debtResult.error);
    }
    db.setChatUser(msg.chat, msg.sender, 'lastwork', Date.now() + cooldown);
    const workMessage = `${pickRandom(trabajo)} *¥${totalReward.toLocaleString()} ${monedas}*.${bonus}`;
    await sock.reply(msg.chat, `ꕥ ${workMessage}`, msg);
  }
};
