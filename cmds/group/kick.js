...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command, groupMetadata, participants }) => {
    ...
    // --- NUEVA VALIDACIÓN DE PROPIETARIOS ---
    const isOwnerTarget = global.owner.map(num => num + '@s.whatsapp.net').includes(targetRaw);
    if (isOwnerTarget) return msg.reply('《✧》 No puedo expulsar a mi dueño.');
    // ----------------------------------------
    if (participant.admin === 'admin' || participant.admin === 'superadmin') return msg.reply('《✧》 No puedes expulsar a otro administrador.');
