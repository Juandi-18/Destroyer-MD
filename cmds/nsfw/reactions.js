  ...
  run: async ({ msg, sock, usedPrefix, command, participants }) => {
    ...
    if (!who) {
      if (participants && participants.length > 0) {
        const choices = participants.map(p => p.id).filter(id => id && id !== sock.user.id && id !== msg.sender);
        who = choices.length ? choices[Math.floor(Math.random() * choices.length)] : msg.sender;
      } else who = msg.sender;
    }
