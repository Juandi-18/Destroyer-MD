...
say('Destroyer', { align: 'center', gradient: ['red', 'blue'] });
say('Made with love by Destroyer', { font: 'console', align: 'center', gradient: ['blue', 'magenta'] });

// CORRECCIÓN CRÍTICA: Añadir la matriz de mapeo para ambas carpetas de subconexiones
const botTypes = [
  { name: 'SubBot Tradicional', folder: './Sessions/Subs', starter: startSubBot },
  { name: 'SubBot Premium', folder: './Sessions/Premium', starter: startSubBot }
];
...
async function loadBots() {
  ...
  try {
    const subsCount = fs.existsSync('./Sessions/Subs') ? fs.readdirSync('./Sessions/Subs').length : 0;
    const premiumCount = fs.existsSync('./Sessions/Premium') ? fs.readdirSync('./Sessions/Premium').length : 0;
...
function cleanCache() {
  ...
  } catch (e) { console.error(chalk.red('Error en cleanCache: '), e); }
}
...
async function warmupGroups(sock) {
  ...
  } catch (e) { console.log(chalk.gray(`[ warmupGroups ] → ${e?.message || e}`)); }
}
...
export async function startBot() {
    ...
    setTimeout(async () => {
      try {
        if (!state.creds.registered) {
          ...
          console.log(chalk.bold.white(chalk.bgMagenta(`[ CÓDIGO DE VINCULACIÓN PRINCIPAL ]`)), chalk.bold.white(chalk.white(codeBot)));
        }
      } catch (err) { console.log(chalk.red("Error al generar código:"), err); }
    }, 3000);
  }

  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    for (const m of messages) { if (m?.key?.remoteJid?.endsWith('@g.us')) { if (!global.msgCache) global.msgCache = {}; if (!global.msgCache[m.key.remoteJid]) global.msgCache[m.key.remoteJid] = []; global.msgCache[m.key.remoteJid].push(m.key); if (global.msgCache[m.key.remoteJid].length > 100) global.msgCache[m.key.remoteJid].shift(); } }
    if (!botReady) return;
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "open") {
      ...
      if (subsBotsPaused) {
        console.log(chalk.gray('[ ✿ ] Reactivando Sub-Bots y Premium Bots después de reconexión del principal...'));
        subsBotsPaused = false;
...
process.on('SIGINT', async () => {
  console.log(chalk.redBright('\n[ ⚠ ] Cerrando socket de Baileys de forma segura para evitar corrupción de sesión...'));
  if (global.sock) { try { global.sock.ws.close(); } catch (err) { console.error('Error cerrando socket en SIGINT:', err); } }
  process.exit(0);
});
